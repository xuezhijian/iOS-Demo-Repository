...
#import "BDAttributedLabel.h"
#import "BDAttributedLabelAttachment.h"
#import "BDAttributedLabelURL.h"

static NSString* const kEllipsesCharacter = @"\u2026";

static dispatch_queue_t bd_attributed_label_parse_queue;
static dispatch_queue_t get_bd_attributed_label_parse_queue() \
{
    if (bd_attributed_label_parse_queue == NULL) {
        bd_attributed_label_parse_queue = dispatch_queue_create("com.bidekeji.BDIos_queue", 0);
    }
    return bd_attributed_label_parse_queue;
}

@interface BDAttributedLabel ()
{
...
@property (nonatomic,strong)    BDAttributedLabelURL *touchedLink;
@property (nonatomic,assign)    BOOL linkDetected;
@property (nonatomic,assign)    BOOL ignoreRedraw;

@end
@implementation BDAttributedLabel
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [_attributedString bd_setFont:_font];
        [self resetFont];
        for (BDAttributedLabelAttachment *attachment in _attachments)
        {
...
- (void)setTextColor:(UIColor *)textColor
{
    if (textColor && _textColor != textColor)
    {
        ...
        [_attributedString bd_setTextColor:textColor];
        [self resetTextFrame];
    }
}
...
- (NSAttributedString *)attributedString:(NSString *)text
{
    if ([text length])
    {
        ...
        [string bd_setFont:self.font];
        [string bd_setTextColor:self.textColor];
        return string;
    }
    else
    {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (BDAttributedLabelURL *url in _linkLocations)
        {
            ...
            [drawString bd_setTextColor:drawLinkColor range:url.range];
            [drawString bd_setUnderlineStyle:_underLineForLink ? kCTUnderlineStyleSingle : kCTUnderlineStyleNone modifier:kCTUnderlinePatternSolid range:url.range];
        }
        return drawString;
    }
    else
...
- (BDAttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            BDAttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    BDAttributedLabelURL *url = [self urlForPoint:point];
...
- (BDAttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (BDAttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (BDAttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
    ...
    callbacks.getAscent     = BDAscentCallback;
    callbacks.getDescent    = BDDescentCallback;
    callbacks.getWidth      = BDWidthCallback;
    callbacks.dealloc       = BDDeallocCallback;
...
{
    [self appendImage:image maxSize:image.size];
}

- (void)appendImage: (UIImage *)image maxSize: (CGSize)maxSize
{
    [self appendImage:image maxSize:maxSize margin:UIEdgeInsetsZero];
}

- (void)appendImage: (UIImage *)image maxSize: (CGSize)maxSize margin: (UIEdgeInsets)margin
{
    [self appendImage:image maxSize:maxSize margin:margin alignment:BDImageAlignmentBottom];
}

- (void)appendImage: (UIImage *)image maxSize: (CGSize)maxSize margin: (UIEdgeInsets)margin alignment: (BDImageAlignment)alignment
{
    BDAttributedLabelAttachment *attachment = [BDAttributedLabelAttachment attachmentWith:image margin:margin alignment:alignment maxSize:maxSize];
...
{
    [self appendView:view margin:margin alignment:BDImageAlignmentBottom];
}

- (void)appendView: (UIView *)view margin: (UIEdgeInsets)margin alignment: (BDImageAlignment)alignment
{
    BDAttributedLabelAttachment *attachment = [BDAttributedLabelAttachment attachmentWith:view margin:margin alignment:alignment maxSize:CGSizeZero];
    [self appendAttachment:attachment];
}
...
{
    BDAttributedLabelURL *url = [BDAttributedLabelURL urlWithLinkData:linkData range:range color:color];
...
{
    [BDAttributedLabelURL setCustomDetectMethod:block];
}
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            BDAttributedLabelAttachment* attributedImage = (BDAttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (void)recomputeLinksIfNeeded
{
    ...
    BOOL sync = length <= BDMinAsyncDetectLinkLength;
...
- (void)computeLink:(NSString *)text sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if ([links count])
        {
            for (BDAttributedLabelURL *link in links)
            {
    ...
    {
        _ignoreRedraw = YES;
        NSArray *links = [BDAttributedLabelURL detectLinks:text];
        block(links);
        _ignoreRedraw = NO;
    }
    else
    {
        dispatch_sync(get_bd_attributed_label_parse_queue(), ^{
...
- (void)addAutoDetectedLink: (BDAttributedLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    BDAttributedLabelURL *touchedLink = [self urlForPoint:point];
