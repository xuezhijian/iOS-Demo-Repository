...
static BDCustomDetectLinkBlock customDetectBlock = nil;

@implementation BDAttributedLabelURL

+ (BDAttributedLabelURL *)urlWithLinkData: (id)linkData range: (NSRange)range color: (UIColor *)color
{
    BDAttributedLabelURL *url  = [[BDAttributedLabelURL alloc]init];
    ...
    else
    {
        ...
        if ([plainText length])
        {
            ...
            [urlRegex enumerateMatchesInString:plainText options:0 range:NSMakeRange(0, [plainText length]) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                        ...
                        BDAttributedLabelURL *link = [BDAttributedLabelURL urlWithLinkData:text range:range color:nil];
